package com.github.skuzi.simplepycharmplugin.predictors
...
class TypePredictor {
    ...
    companion object {
        private const val DEFAULT_PATH = "/home/kuzyaka/Documents/projects/diploma/simple-pycharm-plugin/src/main/resources/models"
        ...
        fun predictType4Py(psiFile: PsiFile, parameters: List<String>): Iterator<String> {
            ...
            val projectFile = Paths.get(DEFAULT_PATH, TYPE4PY_PATH, DEFAULT_PROJECT_DIRECTORY, TMP_AUTHOR, TMP_REPO, DEFAULT_FILE_NAME).toFile()
            ...
            val predictions = DataFrame.readCSV(Paths.get(DEFAULT_PATH, TYPE4PY_PATH, DEFAULT_OUTPUT_DIRECTORY, PREDICTIONS_FILE).toFile())
            val pred = parameters.map { parameter -> predictions.filter { it["name"] eq parameter }["prediction"][0] as String }
        ...
        fun predictDLTPy(psiFile: PsiFile, parameters: List<String>): Iterator<String> {
            val text: String = psiFile.text
            val projectFile = Paths.get(DEFAULT_PATH, DLTPY_PATH, DEFAULT_PROJECT_DIRECTORY, "${TMP_AUTHOR}__${TMP_REPO}", DEFAULT_FILE_NAME).toFile()
            projectFile.createNewFile()
            projectFile.writeText(text)
            ...
            val predictions = DataFrame.readCSV(Paths.get(DEFAULT_PATH, DLTPY_PATH, "output", PREDICTIONS_FILE).toFile())
            val pred = parameters.map { parameter -> predictions.filter { it["name"] eq parameter }["prediction"][0] as String }
