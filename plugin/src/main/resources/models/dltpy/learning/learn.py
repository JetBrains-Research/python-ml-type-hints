...
if __name__ == '__main__':
    ...
    # #for (i, a) in enumerate(y_src_test):
    # #    if 'koalas/frame.py' in a[0] and 'copy' in a[2]:
    # #        print(i)
    #
    # # Start training
    ...
    #
    # print("-- Loading model")
    # model = load_model('/home/kuzyaka/Documents/projects/diploma/dltpy/output/models/load_model/dataset1611806495/model_BiRNN_e_25_l_2.0565700531.h5', model_dir="")
    #
    ...
    #_, config = load_m3()
    model = load_model("model_dltpy.h5")
    t3 = time.time_ns()
    print(t3 - t2)
    ...
    # with torch.no_grad():
    #    Compute model output
       # outputs = model(X.data.to(device))
       # Max for each label
       # y_pred = np.argmax(outputs.data.cpu().numpy(), axis=1)

    y_pred = evaluate(model, test_loader, top_n=1)
    t4 = time.time_ns()
    print(t4 - t3)
